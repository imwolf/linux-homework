...
static int proc_read(char *buf,char **start,off_t off,int count,int *eof,void *data)
{
	...
	*eof = 1; //区别?,不加似乎会在读取结束的时候多发一个不会成功的read请求
	printk(KERN_ALERT "len %d off %d  count %d\n",len ,off,count);
	return len;
}
...
{
	//创建proc/myfile目录
	example_dir = (struct proc_dir_enry *)proc_mkdir("pm1dir",NULL);
	if (example_dir == 0){
		printk ("mkdir fail\n");
		return -1;
	}
	
	data_file = (struct proc_dir_entry *)create_proc_entry("pm1file",0666,example_dir);
	if (data_file == 0){

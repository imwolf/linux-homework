...
int do_part_B(void);
int do_part_C(void);
int do_part_D(char *interation_str,char *duration_str);
int do_part_E();
...
int main(int argc,char *argv[])
{
	...
	if (argc > 1){
		...
		if (c1 != '-' || c2 == NULL){
			printf("Usage:./observer [-a] [-b] [-c] [-d int dur]\n");
			exit(1);
		}
	}
	...
	if (c2 == 'd'){ //观察部分D
		...
		return do_part_D(argv[2],argv[3]);
	}	
}
...
int do_part_D(char * interval_str,char * duration_str)
{
